...
label_propagation = label_propagation.fit(X_labeled, Y_labeled)
Y_unlabeled_pred = label_propagation.predict(X_unlabeled)
print(Y_unlabeled_pred)
X_combine = np.concatenate((X_labeled, X_unlabeled))
Y_combine = np.concatenate((Y_labeled, Y_unlabeled_pred))


decision_tree = DecisionTreeClassifier(criterion="entropy", max_depth=35, min_samples_split=20, min_samples_leaf=5)
decision_tree.fit(X_combine, Y_combine)
